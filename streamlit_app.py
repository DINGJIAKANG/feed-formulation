...
<div class="app-header">
    <h1>🌾 线性规划饲料配方优化系统</h1>
    <div class="subtitle">基于线性规划的最低成本饲料配方求解工具 | 支持5类动物 | 多种约束灵活配置</div>
    <div class="stats">
...
| 步骤 | 操作 |
|------|------|
| **① 选参数** | 左侧选「动物种类」→「饲养标准」→ 选择原料（全部/手动）|
| **② 设约束（可选）** | 侧边栏底部「高级设置」：多种约束灵活配置 |
| **③ 求解导出** | 点击「▶ 开始求解」→ 查看/微调/导出 Excel |

---

### 🔧 多种约束（程序核心特点）

本程序支持 **4 大类约束**，满足实际配方设计的复杂需求：

| 约束类型 | 说明 | 设置位置 |
|----------|------|----------|
| **营养上下限** | 对钙、磷、赖氨酸等任意营养指标设上限或下限 | 高级设置 → 营养上限约束 |
| **原料用量限制** | 对特定原料设最小/最大用量（如食盐 0.3%~0.5%）| 高级设置 → 用量限制 |
| **精粗比约束** | 限制精料与粗料的比例范围（反刍动物自动启用）| 高级设置 → 精粗比约束 |
| **钙磷比约束** | 限制钙磷比值范围（如 1.5:1 ~ 2.5:1）| 高级设置 → 钙磷比约束 |

> 💡 **提示**：约束越多，求解难度越大。若求解失败，尝试放宽部分约束。

---

### 📌 关键概念

**干物质基础 (DM)**：去除水分后计算，保证不同饲料可比。程序自动处理 DM 转换，结果中「DM配方」用于分析，「投料单」用于生产称重。

**原料价格**：未填价格的原料默认设为 999（高价，LP 会避免使用）。如需强制使用某原料，在「高级设置 → 用量限制」中设下限即可。

**自动补充**：当前原料无法满足需求时，程序会从 548 种内置原料库自动筛选补充。
...
**Q：上传文件后没反应？**
检查格式（.xlsx/.xls）、第1列是否为原料名称、列名是否与模板一致

**Q：为什么某些原料没被选中？**
价格缺失(999) → 填写价格；NPN 原料(尿素) → 单胃动物自动排除；缺少对应能量列

**Q：线上版和本地版有区别吗？**
算法功能完全相同。线上版免安装随时访问；本地版支持改代码和大文件
""")
