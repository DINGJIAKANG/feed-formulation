...
| 步骤 | 操作位置 | 说明 |
|------|----------|------|
| **① 选参数** | 左侧「数据文件」+「配方设置」 | 选择动物、饲养标准、原料 |
| **② 设约束（可选）** | 「高级设置」（侧边栏底部） | 设置营养上限/精粗比/用量限制 |
| **③ 求解导出** | 点击「开始求解」按钮 | 查看/微调/导出结果 |
...
2. 选择你要配方的目标动物：
   ...
   - **羊** → 使用消化能
   - **其他**（猫/狗/鱼/马）→ 使用消化能
3. 选择后，系统自动加载对应的饲养标准列表

4. 在 **「饲养标准」** 下拉框中选择具体标准：
   - 例如：「猪-生长肥育猪-20-50kg」、「肉鸡-0-3周龄」等
   - 标准会显示该阶段的营养需求（能量、粗蛋白、钙、磷等）
...
| 选项 | 适用场景 | 说明 |
|------|----------|------|
| **干物质基础(DM)** ✅ 推荐 | 大多数情况 | 标准值已是干物质基础，直接使用 |
| **风干饲粮基础** | 你的标准是风干物基础 | 系统会自动 ÷DM% 转为 DM 基础 |

> 💡 **什么是干物质基础？**
> 饲料中含有水分（如玉米约含14%水），不同饲料水分差异很大。
> 为保证配方可比性，统一以「去掉水分后的物质（DM）」为基准计算。
>
> 典型 DM% 参考值：
> - 配合饲料：~88%
> - 苜蓿干草：~90%
...
1. 选择 **「🔘 使用全部可用原料」**
2. 系统从内置的 **548 种原料数据库** 中自动筛选适合该动物的原料
3. 单胃动物（猪/禽）会**自动排除**尿素等非蛋白氮原料
4. 直接点击「开始求解」即可

#### 方式 B：手动选择原料

1. 选择 **「🔘 手动选择原料」**
2. 在下方出现的多选框中勾选要使用的原料
3. 可以使用搜索框快速查找原料名称
4. 原料名称格式为 **「原料名[等级]」**，例如：
   - `玉米[2级8%]`
   - `大豆粕[2级44.2%]`
...
如果你有自己的原料价格或特殊原料，可以上传自定义数据：

1. 点击 **「📥 上传数据文件」**
2. 下载模板文件（**原料数据模板.xlsx**）
3. 按模板格式填写你的原料数据
4. 上传填写好的文件

> ⚠️ **重要提示**：
> - 第1列必须是 **原料名称**（与数据库一致才能匹配）
> - 第2列填 **价格**（元/kg），不填则默认设为999（避免被使用）
> - 其他列为营养成分（粗蛋白%、钙%、总磷% 等）
> - 列名必须与模板完全一致！
...
#### 4.2 原料用量限制
- 设置某些原料的最小/最大使用量
- 例如：玉米 ≥ 30%（确保基本能量供应）、鱼粉 ≤ 5%（控制成本）
- 操作：选择原料 → 输入下限和上限

#### 4.3 精粗比约束（反刍动物专用）
- 控制精料（谷物/粕类）与粗料（牧草/秸秆）的比例
- 牛默认范围：25%~75%（精料比例）
- 羊默认范围：20%~70%
- 如果手动设置，将覆盖默认值

#### 4.4 钙磷比约束
- 保证钙与磷的比例在合理范围内（通常 1:1 ~ 2:1）
- 有助于骨骼健康和代谢平衡
...
| 状态 | 含义 | 建议 |
|------|------|------|
| **✅ 最优解** | 成功找到最低成本配方 | 继续查看/微调/导出 |
| **❌ 无解** | 约束条件互相冲突 | 放宽某些约束 |
| **⚠️ 自动补充** | 当前原料无法满足需求 | 系统已自动补充原料 |
...
2. 文件包含多个工作表：
   - **配方组成**：各原料配比及成本
   - **营养对比**：需求 vs 实际
   - **投料单**：原样基础(as-fed)的实际投料重量

> 💡 **投料单说明**：
> DM配方 × DM% = 实际投料重量
> 例如：玉米 DM配比60%，DM%=88% → 实际投料 = 60%×88%/Σ(各原料×DM%)
...
### Q1：求解失败/无解怎么办？
**A：** 常见原因和解决方法：
- **约束冲突**：如要求玉米≥80% 且豆粕≥30%（两者之和超过100%）→ 降低某一下限
- **营养不可行**：如要求粗蛋白25%但最高蛋白原料只有20%→ 放宽要求或换高蛋白原料
- **原料太少**：只选了3-4种原料 → 增加原料种类或开启「自动补充」
...
### Q3：如何添加其他动物的营养需求？
**A：** 方法一：上传自定义饲养标准文件（按模板格式）
方法二：在程序内置的标准数据库中添加新标准

### Q4：为什么有些原料没有被选中？
**A：** 可能的原因：
- 价格设为999（缺失价格自动设为高价）→ 填写正确价格
- 是NPN原料（如尿素），单胃动物自动排除 → 反刍动物可用
- 不符合该动物的能量体系（如缺少禽代谢能列）

### Q5：如何让某种原料必须使用？
**A：** 两种方式：
1. 在「高级设置：原料用量限制」中设置**下限** > 0
2. 即使价格为999，设置了下限也会强制使用

### Q6：DM配方和投料单有什么区别？
**A：**
- **DM配方**：去除了水分的纯营养物质配比（用于比较和分析）
- **投料单**：考虑水分后的实际称重（用于生产操作）
- 程序先优化 DM 配方 → 再转换为投料单

### Q7：为什么配方成本很高？
**A：** 检查以下几点：
1. 是否有高价格原料（如鱼粉、血浆蛋白粉）占比过高
2. 营养要求是否过高（如粗蛋白22%以上）
3. 原料限制是否过严（排除了廉价原料）
4. 可尝试放宽约束或增加低价原料

### Q8：线上版和本地版有什么区别？
**A：**
- **线上版**（Streamlit Cloud）：无需安装，随时访问，免费使用
- **本地版**：需要安装Python环境，可以修改代码，支持大文件
- 两者的算法和功能**完全相同**
...
1. **首次使用建议**：先用「使用全部可用原料」模式跑一次，了解系统的基本能力
2. **逐步添加约束**：不要一次性设置太多限制，容易导致无解
3. **关注关键营养**：优先保证能量、粗蛋白、赖氨酸等核心指标
4. **保存好配方**：每次求解后及时导出 Excel 备份
5. **定期更新价格**：原料市场价格波动较大，及时更新可以获得更准确的成本估算
""")
